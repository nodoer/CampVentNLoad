 ...
 * ----------------------------------- */
 // 1.1 Added clock set
 // Dew Point diff set
 // battery threshold voltage sets
 // 1.2 Make ciculation fans come on when battery is at 13.8
 // Try to fix back light
 // Entering Bypass Mode will turn off fans if they are running
 // 1.3 Remove circulate functionality
...
void setup() {
  ...
  lcd.clear();
  lcdPrintLines("Shiroda Power Co", "2018        V1.6");
...
void loop() {
  ...
  if(timeState.minute() != state.currentMinute){
    updateTimers();
    state.currentMinute = timeState.minute();
  }
  ...
  //check to see if it is 7 or 19 and bypass is off to run circ fans
  if((timeState.hour() == 9 || timeState.hour() == 19) && !axState.rly2MinTimer && !state.bypassMinTimer){
